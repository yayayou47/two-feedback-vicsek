# Physical Review Research submission package

Target: **Physical Review Research** (APS), original research article.
...
| File | Role |
|------|------|
| `cover_letter.{tex,pdf}` | Cover letter to the Physical Review Research editorial board |
| `manuscript.tex` (+ `.pdf`) | Main article, **REVTeX 4.2** (`aps,prresearch`) — **compiles here** with `apsrev4-2` |
...
The manuscript uses `\documentclass[aps,prresearch,preprint,amsmath,amssymb,nofootinbib,superscriptaddress]{revtex4-2}`
...
If a given REVTeX build rejects the `prresearch` journal option, use `pre`
(sibling PR journal) or generic `aps`; only minor formatting changes.
