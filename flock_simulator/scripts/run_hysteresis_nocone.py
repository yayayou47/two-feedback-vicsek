"""Hysteresis loop in eta.

The U_4 dip leaves the order of the transition ambiguous (the
manuscript calls it "at most weakly first-order"). A quasi-static
eta ramp settles it: a continuous transition retraces the same
phi(eta) curve up and down, while a first-order transition opens
a hysteresis loop. We ramp eta up from deep order to disorder and
back without re-randomising between steps (each eta inherits the
previous configuration), holding many steps per eta to relax.

Modes: motility-only and double-adaptive (the two motility-active
modes) at L = 64, where the synergy is sharpest. Output:
data/double_hysteresis_nocone.npz with phi on the up- and
down-sweep per mode, averaged over seeds, plus the loop area.
"""
...
def main() -> None:
    ...
    # eta ramp up then down over the critical window
    ...
    pbar = tqdm(total=n_modes * n_seeds, desc="hysteresis_nocone")
    t0 = time.time()
    for im, (name, vmn, vmx, amn, amx) in enumerate(cases):
        for isd, seed in enumerate(seeds):
            ...
            sim.state.theta[:] = 0.0
            for _ in range(1500):                 # initial warm-up
                sim.step()
            sweep(sim, eta_up, phi_up, im, isd)    # up-ramp
            sweep(sim, eta_down, phi_down, im, isd)  # down-ramp
            pbar.update(1)
    pbar.close()

    phi_up_m = phi_up.mean(axis=1)
    phi_up_se = phi_up.std(axis=1, ddof=1) / np.sqrt(n_seeds)
    phi_dn_m = phi_down.mean(axis=1)[:, ::-1]      # re-order to eta_up grid
    phi_dn_se = phi_down.std(axis=1, ddof=1)[:, ::-1] / np.sqrt(n_seeds)
    # loop area as |trapz(phi_down - phi_up)| over eta
    loop_area = np.array([
        abs(np.trapezoid(phi_dn_m[im] - phi_up_m[im], eta_up))
        for im in range(n_modes)
    ])

    out = DATA / "double_hysteresis_nocone.npz"
    np.savez_compressed(
        out,
    ...
    print(f"\n{'mode':>10s}  {'loop area':>10s}")
    for im, c in enumerate(cases):
        print(f"  {c[0]:>10s}  {loop_area[im]:8.4f}")
