"""Fine-L s_sep scan for ALL FOUR modes.

Extends run_double_Lfine_nocone.py (motility + full only) with the
two fixed-speed references (baseline / noise-shape) so Fig 7 can
show every mode. Sizes {38, 50, 60, 75, 105}, 4 etas, 5 seeds.
Output: data/double_Lfine_allmodes_nocone.npz, same schema as the
legacy Lfine file but with four modes.
"""
...
def main() -> None:
    ...
    pbar.close()
    out = DATA / "double_Lfine_allmodes_nocone.npz"
    np.savez_compressed(out,
                        modes=np.array([m[0] for m in modes]),
                        Ls=Ls, etas=etas,
                        phi=phi, chi=chi, s_sep=s_sep,
                        params=np.array([sigma, n_warm, n_meas,
                                         len(seeds)]))
    print()
