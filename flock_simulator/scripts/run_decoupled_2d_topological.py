"""
Topological-alignment companion to ``run_decoupled_2d_nocone.py``.

Same protocol (L = 30, sigma = 2.22, 25-cell grid
(n_star_v, n_star_alpha) in {1, 2, 3, 5, 8}^2, motility-only
reference, ten seeds per cell, eta = 0.15) but with the
alignment kernel switched to k = 4 nearest non-repulsion
neighbours. The density-sensing channel (the metric n_ali fed
into the shared sigmoid) is unchanged: only the alignment
direction kernel differs from the metric run.

If the motility-threshold dominance reported in
``run_decoupled_2d_nocone.py`` survives under topological
alignment, the finding is universal in the alignment kernel.
If it disappears or weakens substantially, the dominance is a
specific metric-alignment property.

Output: data/double_decoupled_2d_topological.npz with the same
schema as the metric run.
"""
...
def run_one(nsv, nsa, alpha_min, alpha_max, seeds, n_warm,
             n_snap, n_skip, L, N, eta, r_edges, n_bins, R_local):
...
def main() -> None:
    ...
    out = DATA / "double_decoupled_2d_topological.npz"
    np.savez_compressed(
        out,
        ...
        alignment_kernel="topological_kNN4",
    )
    print()
    ...
    print(f"\nDelta_g(r ~ 0.62) topological vs motility-topo, "
          f"10 seeds:")
    print(f"{'n_v\\n_a':>10s} " + " ".join(
