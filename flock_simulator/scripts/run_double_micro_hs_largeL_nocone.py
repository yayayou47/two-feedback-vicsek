"""No-cone port of legacy run_double_micro_hs_largeL.py.

10-seed dense/dilute g(r) + cluster sweep at L in {64, 90} for
motility-only and full modes. Feeds the persistence test for
the dense-quartile g(r) signature.
"""
...
def main() -> None:
    ...
    n_modes, n_L, n_seeds = len(modes), len(Ls), len(seeds)
    ...
    pbar = tqdm(total=n_modes * n_L * n_seeds, desc="micro_hs_largeL_nocone")
    t0 = time.time()
    for im, (label_name, vmn, vmx, amn, amx, eta) in enumerate(modes):
        for iL, L in enumerate(Ls):
            ...
            for isd, seed in enumerate(seeds):
                ...
                gr_dense[im, iL, isd] = np.where(cD > 0, sD / cD, np.nan)
                gr_dilute[im, iL, isd] = np.where(cL > 0, sL / cL, np.nan)
                pbar.update(1)
    pbar.close()

    out = DATA / "double_micro_hs_largeL_nocone.npz"
    np.savez_compressed(
        out,
        modes=np.array([m[0] for m in modes]),
        Ls=Ls, seeds=np.array(seeds), r_centers=r_centers,
