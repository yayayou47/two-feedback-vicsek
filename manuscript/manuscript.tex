...
single-channel ablations --- a motility-only mode (adaptive
speed, fixed Cauchy noise) and a noise-shape-only mode (fixed
speed, noise interpolating from Cauchy to Gaussian) --- isolate
...
\citep{Binder1984}. For spatial phase separation we use
\begin{equation}
...
sizes onward (per-seed paired $t = +5.9$ at nine d.o.f.). With
ten seeds the $\chi_{\max}(L)$ series is monotone for both
motility-active modes, the non-monotonic $L = 90$ node of the
earlier three-seed survey a seed-budget artefact. The pooled
estimator stays seed-sensitive at large $L$ under heavy tails,
which is why the microscopic $g(r)$, $C(\tau)$, GNF and $P(s)$
diagnostics below carry the load-bearing evidence.
...
the three largest sizes, $s_{\rm sep} = (1.25, 1.25, 1.48,
1.72)$ at $L = 64$, $(1.19, 1.18, 1.68, 1.78)$ at $L = 90$, and
...
$+0.24$ at $L = 64$, $+0.10$ at $L = 90$ and $+0.01$ at $L =
128$ as they converge on the plateau. So $s_{\rm sep}$ reads the
same split as the susceptibility slope, with a noise-shape
surplus largest at intermediate sizes before the plateau closes
it.

The snapshots (Fig.~\ref{fig:snapshot}) make the contrast
concrete. At $\eta = 0.020$ the two modes are almost
indistinguishable, $\langle\varphi\rangle = 0.78$ and $0.83$; at
near-critical $\eta = 0.10$ the double-adaptive model already
holds more order ($\langle\varphi\rangle = 0.47 \to 0.63$); and
at $\eta = 0.30$ the motility ablation collapses to
$\langle\varphi\rangle = 0.04$ while the double-adaptive model
still holds $0.25$, its dense slow patches staying coherently
aligned where the motility-only patches scramble. Inside the
...
peaking at $(n_\star, s) \approx (3.3, 8.5)$, so the cohesion
...
at $(n_\star, s) \approx (1, 4.75)$ with $\chi_{\max} = 46$; the
whole $n_\star \le 2$ rectangle ($\langle\chi\rangle \approx
16$) is brighter than the $n_\star \simeq 3$ row ($\approx 12$),
and the $\chi_{\max}$ peak sits where $s_{\rm sep}$ has dropped
...
reaching $\chi_{\max} = 136$ at $L = 90$ and $104$ at $L = 128$,
as expected for a growing phase-separation susceptibility,
while the density-separation maximum stays in the low-threshold
band $n_\star \lesssim 4$, peaking at $s_{\rm sep} = 1.94$ ($L =
90$) and $1.82$ ($L = 128$) with the $n_\star \ge 5$ rectangle
still suppressed near $s_{\rm sep} \simeq 1.2$. The cohesion
...
ten-seed paired gap at the full-mode $\chi$-peak $\eta$ for $L
\in \{50, 64, 80\}$ (Table~\ref{tab:gap_se}) gives $+0.020 \pm
0.032$, $+0.165 \pm 0.031$ and $+0.024 \pm 0.031$: only $L = 64$
...
fluctuations ($2.5 \pm 0.4$ and $2.9 \pm 0.5$ clusters per seed,
max size near the threshold floor). Motility-only fragments the
dense phase into $187.9 \pm 12.3$ clusters, max size $55.0 \pm
7.5$, mean $9.7 \pm 1.3$; the full mode gives $162.0 \pm 14.5$
clusters, max size $176.6 \pm 42.3$, mean $22.6 \pm 6.5$. Taking
the seed as the unit of replication, and because the
dense-phase estimators are heavy-tailed, we report each
full-minus-motility gap with its $95\%$ seed-bootstrap CI ($B =
10\,000$) and a Wilcoxon signed-rank test rather than a Gaussian
$z$. The two motility-active modes differ on cluster count by
$-25.9$ ($[-62.9, +19.5]$, $p = 0.19$; not significant) and on
max size by $+121.6$ ($[+40.5, +206.6]$, $p = 0.02$): the
rectification fuses dense patches into a few large clusters at
conserved total dense area (Fig.~\ref{fig:cluster_map}). Max
size is the meaningful diagnostic; the count carries no clean
signal.
...
too weak to nucleate a dense phase with the motility channel off
--- while both motility-active modes carry one. At their
...
test gives $\Delta g(r \!\simeq\! 0.6) = +0.150$ ($[+0.131,
+0.167]$) at $L = 64$, $+0.127$ ($[+0.098, +0.157]$) at $L =
90$, and $+0.106$ ($[+0.082, +0.133]$) at $L = 128$ (each $95\%$
seed-bootstrap CI excluding zero, Wilcoxon $p = 0.002$); with
$L = 30$ the dense-quartile gap plateaus at $\Delta g \in
[+0.11, +0.15]$, narrowing mildly from $+0.15$ at $L \le 64$ to
$+0.11$ at $L = 128$. Over the full $r \in [0.5, 6]$ profile
(Fig.~\ref{fig:gr}c) it does not decay with $r$: the inner-range
mean $r \in (0.5, 1.5)$ sits at $+0.150, +0.152, +0.131, +0.113$
for $L = 30, 64, 90, 128$, while the outer-range mean $r > 3$
climbs to $+0.181, +0.184, +0.157, +0.137$ --- if anything
larger beyond the alignment cutoff $R_a = 0.7$ than inside it.
The rectification imposes a coherence on the dense phase that
...
over $\tau \in [1, 2\,000]$ in the dense quartile, with
bootstrap $95\%$ CIs (ten seeds, $B = 10\,000$ resamples)
excluding zero throughout; at $\tau = 5\,000$ it drops to
$+0.18$, CI $[+0.08, +0.27]$, still excluding zero. The
within-seed estimator averages over $\sim 4 \times 10^7$ sample
pairs per mode, so the between-seed SE is $\sim 10^{-3}$ and the
$t$-statistic at nine d.o.f.\ saturates beyond a few sigmas;
hence the bootstrap CIs. The dilute gap is a few percent and its
CI brackets zero past $\tau \simeq 100$.
...
directed inside the dense flock, heavy-tailed in the dilute gas
--- the speed channel sets the magnitude, and together they keep
...
$\langle n_i\rangle_{\rm dense} = 2.57, 4.05, 4.85, 5.26$ at
$\rho_0 = 1.0, 1.5, 2.22, 3.0$ \emph{increases} with $\rho_0$,
...
softens. The linear fit on $n_\star^v$ alone now captures $R^2 =
0.68$ (was $0.88$), while the $n_\star^\alpha$ fit climbs to
$R^2 = 0.43$ (was $0.04$); the bivariate fit $\Delta g \simeq
-0.081\,n_\star^v - 0.069\,n_\star^\alpha + 0.73$ splits the
weight nearly evenly between the two thresholds, against the
$-0.144/-0.029$ split under metric alignment. The sign-flip
region retreats too: $(5,1)$ and $(8,1)$, which gave $-0.27$ and
$-0.63$ under metric, now give $+0.31$ and $+0.21$, only the
high-high corner staying negative.
...
their own grow no size-dependent $\chi_{\max}(L)$ at our density
--- the alignment averaging in the dense phase washes the bursts
out. The motility channel is necessary, heavy tails alone are
...
short: the $\Delta_n$ excess is positive at every budget and
excludes zero from $n = 5$ onward ($\Delta_7 = +0.63$, $[+0.40,
+0.83]$), but the $\chi_{\max}$ peaks already flatten at the
largest sizes, so we read it as a finite-size boost rather than
an asymptotic exponent. Controlled runs at $L = 180$ or $256$,
with an explicit data collapse, would test whether the excess
slope holds asymptotically and whether $P(\langle\varphi\rangle)$
narrows. Beyond that, mapping $g(r)$ at intermediate distances
$r = 2$ to $5$ would tell whether the rectification carries a
characteristic decay length, in the spirit of
\citet{Eliazar2003}; a two-knob sweep over $v_{\min}/v_{\max}$
and $\alpha_{\min}/\alpha_{\max}$ at fixed $(n_\star, s)$ would
isolate each channel's marginal contribution and bridge to the
Vicsek limit; and the absence of a traveling band leaves room
for a different cohesive structure, perhaps a slowly-rotating
dense droplet, that a real-space analysis at $L > 30$ should
reveal. On the theory side, a Toner--Tu closure with
$v_{\rm eff}(\rho)$ and a fractional $D_{\rm rot}(\rho) \propto
\eta^{\alpha(\rho)}$ recovering both the linear $\Delta
g(n_\star^v)$ and the negative slope $n_\star^{v,{\rm
crit}}(\rho_0) = -0.73\,\rho_0 + 5.81$ would close the
mechanistic argument; we plan to fit $\alpha_{\rm eff}(\rho)$ and
$v_{\rm eff}(\rho)$ directly and benchmark the effective
$\gamma/\nu$ against the metric Vicsek prediction.
...
The treatment is heuristic --- the aim is to make the
rectification analytically transparent, not to derive it.
...
has characteristic function $\exp(-\eta^\alpha |k|^\alpha)$, so
...
Decoupling the sigmoid replaces Eq.~\eqref{eq:sigma_rho} by two
independent profiles with thresholds $n_\star^v$ and
$n_\star^\alpha$, and a sequential timing argument
(motility-first $\succ$ shared $\succ$ alpha-first) captures only
the diagonal ordering. It misses the empirical map
(Fig.~\ref{fig:decoupled_2d}), which runs along horizontal lines
of constant $n_\star^v$ ($R^2 = 0.88$ against $R^2 = 0.28$ for
the timing-offset fit), with $(5,8)$ staying negative though
motility-first. The limit is real: the linear-stability sketch
reproduces neither the horizontal-band structure of the
...
\eqref{eq:p_eq} would need the dense-phase local density tied to
the MIPS contrast rather than to the mean, which we leave to a
quantitative theory.
