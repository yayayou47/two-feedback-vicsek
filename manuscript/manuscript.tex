...
mode, an excess $\Delta_7 = +0.63$, $95\%$ seed-bootstrap
interval $[+0.40, +0.83]$ excluding zero, over the accessible
sizes);
...
fixed density $\rho_0 = N/L^2 = 2.22$, with $1500$ warm-up and
$1000$ measurement steps from $\theta_i \equiv 0$. The
load-bearing finite-size slopes (Table~\ref{tab:slopes} and the
$\chi_{\max}(L)$ panel of Fig.~\ref{fig:pilot}) come from a
homogeneous protocol with ten seeds at \emph{every} size, so the
bootstrap slope intervals rest on a uniform replication budget;
the $\eta$-resolved diagnostic curves (the remaining panels of
Fig.~\ref{fig:pilot}) use three seeds per
$(\mathrm{mode}, L, \eta)$ cell. The noise grid is
...
\begin{figure}[htbp]
    ...
    four modes: Cauchy reference (blue), noise-shape
    only (green), motility only (red), double-adaptive (purple).
    The $\eta$-resolved panels (three seeds) show, versus $\eta$ at
    the largest $L$: (a) polar order
    $\langle\varphi\rangle$, (b) susceptibility $\chi$, (c)
    $s_{\rm sep}$, (d) Binder cumulant $U_4$; (e) log--log
    $\chi_{\max}$ versus $L$ with fitted slopes, on the ten-seed
    homogeneous series of Table~\ref{tab:slopes}; (f) mean speed
...
between the channels, not a proven asymptotic exponent.
Figure~\ref{fig:collapse} makes both points directly: the local
slope $a_{\rm eff} = d\ln\chi_{\max}/d\ln L$ overshoots its
global fit at intermediate sizes and falls back to
$a_{\rm eff} \simeq 0$ in the largest interval, and the
susceptibility peak $\eta^\ast(L)$ migrates toward the
small-$\eta$ boundary rather than locking to a fixed $\eta_c$,
so a $(\eta - \eta_c)L^{1/\nu}$ collapse cannot be defined.
...
\alpha_{\max} = 2$ on the same seven-size grid as the
heavy-tailed modes (its row in Table~\ref{tab:slopes}).
It develops no critical $\chi_{\max}(L)$ scaling at our density:
the peak stays in $1.0$--$1.2$ across the seven sizes, slope
$a_{\rm Vicsek} = -0.04$ ($[-0.08, +0.01]$), the metric-Vicsek
...
exponent. The core thermodynamic and microscopic evidence is
also taken at a single global density ($\rho_0 = 2.22$); only the
decoupled-sigmoid map probes the density axis, so whether the
dense-quartile gap and the FSS excess survive a sweep in
$\rho_0$ is open. Controlled runs at $L = 180$ or $256$ with an explicit
