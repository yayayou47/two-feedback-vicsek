...
super-additive excess that grows over the accessible sizes but
saturates rather than signalling a critical divergence. The
robust fingerprint of the coupling is microscopic: the
...
staying passive. The coupling thus leaves its most reliable mark
not in the bulk susceptibility but in the density-stratified
turning statistics of the dense phase, a fingerprint that should
be accessible in crowding-responsive metric collectives.
\end{abstract}
...
super-additive boost ($\chi_{\max} \sim L^{1.49}$ for the full
mode, an excess $\Delta_9 = +0.44$, $95\%$ seed-bootstrap
interval $[+0.31, +0.60]$ excluding zero, over the nine accessible
sizes);
this growth saturates toward the $d = 2$ ceiling
($\chi_{\max} \sim N$ for a single dense domain) rather than
marking a critical exponent, so we read it as a finite-size,
not an asymptotic, effect, and the robust evidence for the
coupling is microscopic rather than thermodynamic. The mechanism
is local.
...
$[+0.31, +0.60]$, excluding zero. Extending the lever shrinks
the excess (the seven-size fit gave $\Delta_7 = +0.63$) but does
not close it: the super-additive cooperation between the
channels survives the larger boxes even as the individual slopes
drift down, which is the signature of a finite-size boost rather
than a stable asymptotic exponent.
...
at large $L$ under heavy tails, which is why the microscopic
$g(r)$, $C(\tau)$, GNF and $P(s)$ diagnostics below carry the
...
builds and that phase grows with density. The $\rho_0 = 3.0$
full slope $+3.00$ exceeds the $d = 2$ ceiling
$\chi_{\max} \le N$, a direct reminder that these are finite-size
crossover rates, not asymptotic exponents.
...
sizes. The susceptibility synergy therefore carries the cleanest signature
...
carries a positive, density-tunable super-additive excess that
survives to nine sizes (Table~\ref{tab:density}). Its microscopic
face is one rectification mechanism made visible: the dense
$g(r)$, its dynamic counterpart $C(\tau)$ and the cluster
geometry all express the lower effective noise the noise-shape
feedback installs in the dense phase, flat in the dilute
quartile, and a matched-$\Gamma$ control reverses the $g(r)$ and
$C(\tau)$ gaps, so these are not independent coherences but the
same noise rectification seen statically and dynamically. The
susceptibility excess is a separate, growth-rate effect we have
not retested at matched $\Gamma$. The
...
+1.04$) and shrinks the synergy excess ($\Delta_7 = +0.63$ to
$\Delta_9 = +0.44$, $[+0.31, +0.60]$, still excluding zero): the
cooperation survives the larger boxes but does not stabilise
into an exponent, which is exactly the finite-size reading we
adopt. The split is moreover not a parameter-window effect, the
motility-active growth and the noise-shape excess both
