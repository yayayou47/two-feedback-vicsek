...
the channels becoming additive. The two feedbacks therefore do
...
the lower dense effective noise of its rectified kick made
visible, the same mechanism the dense $g(r)$ and $C(\tau)$ gaps
below express; at matched effective noise the two channels are
additive. (The retuned mode is also globally quieter, so this
...
so this each-own-$\eta$ excess does not hinge on the non-monotone
tail (its noise-level origin is established above). The
...
$L = 180, 256$; both that gap and the $\rho_0 = 3.0$ value
$+3.00$, which exceeds the $d = 2$ ceiling $\chi_{\max} \le N$
that any true exponent must respect, confirm that these are
finite-size crossover rates rather than asymptotic exponents, so
we read the density trend qualitatively, the excess growing with
$\rho_0$, rather than as a series of exponents.
...
signature here, so we turn instead to $s_{\rm sep}$ for the
spatial reading.
...
not sufficient, and what the noise-shape adaptation buys is not a
cooperative excess on top of the motility-driven scaling but a
lower effective noise inside the dense phase, which raises
coherence and susceptibility together by the one route a
