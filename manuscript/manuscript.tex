...
kicks into Gaussian ones, switching dense reorientation from
heavy-tailed to Gaussian, sharpening the local heading
...
control of the sign and amplitude of the rectification, with the
noise-shape threshold staying passive, and the dominance is a
property of the metric kernel. The coupling thus leaves its mark
as a density-gated noise rectification rather than an emergent
thermodynamic synergy, a signature that should be accessible in
crowding-responsive metric collectives.
\end{abstract}
...
Gaussian and an isolated one fast and Cauchy. In plain terms the
stability index sets how heavy-tailed the turning is: the Cauchy
limit ($\alpha = 1$) sprinkles in occasional near-reversals,
while the Gaussian limit ($\alpha = 2$) keeps every turn small,
and motility-induced phase separation (MIPS) is simply clustering
that arises because particles pile up where crowding slows them,
with no attraction needed. This single shared
sensor is a deliberate minimal-coupling assumption rather than a
documented mechanism: a speed--density anti-correlation and
...
convenient family bracketing the Cauchy and Gaussian limits. As
the analysis below shows, the effect is moreover a property of
...
motility-only, over nine sizes up to $L = 256$); heavy-tailed noise on its own leaves the
transition essentially Vicsek-like, its rare bursts washed out
by the many neighbours of a dense patch. Compared at each mode's
...
excess is not, however, a cooperation beyond noise: a
...
correlation gaps below. The two channels therefore act through a
...
\begin{figure}[htbp]
    ...
    Eq.~\eqref{eq:theta_star}, each at $t$ and $t + \delta t$. A
    single neighbour-count sigmoid thus ties slow motion to
    Gaussian turning and fast motion to Cauchy turning, the one
    departure from canonical Vicsek dynamics.}
    \label{fig:schematic}
\end{figure}
...
bootstrap intervals rest on a uniform replication budget;
the $\eta$-resolved diagnostic curves (the remaining panels of
...
the scan at two further densities $\rho_0 \in \{1.0, 3.0\}$
(sizes $22$ to $128$). The noise grid is
...
\begin{figure*}[htbp]
    ...
    deliberate departure from a geometry-only snapshot here because
...
\begin{figure}[htbp]
    ...
    $\langle\alpha_i\rangle$ (dotted). Only the two motility-active
...
($[-0.05, +0.01]$, $[-0.07, +0.01]$ and $[-0.05, +0.01]$): none
...
single phase-separated domain would in any case saturate at the
...
interval $[+0.31, +0.60]$, when each mode is taken at its own
near-critical noise (and robust to a size-jackknife, below).

This apparent excess is, however, a noise-level effect, not a
...
$\eta \to \eta^2$ retune also quiets the dilute phase. All three
observables thus lose the full-mode advantage once the dense
decorrelation is equalised: the faster $\chi_{\max}$ growth, the
sharper dense $g(r)$ and the slower dense $C(\tau)$ are one
effect, the lower dense effective noise of the rectified kick
made visible, not a cooperation beyond noise. The retune bounds
this as a noise effect rather than isolating the dense phase
alone, since it lowers noise everywhere.
...
than locking to a fixed $\eta_c$, so a $(\eta - \eta_c)L^{1/\nu}$
collapse cannot be defined.
The load-bearing evidence is the microscopic signature below.
...
to $+1.15$ or $+1.07$, so the excess $\Delta = a_{\rm full} -
a_{\rm motility}$ stays in $[+0.45, +0.52]$ under either deletion.
This each-own-$\eta$ excess therefore does not hinge on the
non-monotone tail (its noise-level origin is established above). The
microscopic $g(r)$, $C(\tau)$, GNF and $P(s)$ diagnostics below
carry the load-bearing evidence.
...
$\rho_0 = 1.0$ ($+0.63$ versus $+0.64$), separate by
$\simeq +0.8$ at $\rho_0 = 2.22$, and by $+1.0$ at
$\rho_0 = 3.0$ ($+3.00$ versus $+2.00$), exactly as expected if
the rectification acts inside the dense phase the motility
builds and that phase grows with density. These common-range
($L = 22$ to $128$) slopes run steeper than the nine-size values
of Table~\ref{tab:slopes} (where $a_{\rm full} = +1.49$ at
$\rho_0 = 2.22$) because they omit the two saturating sizes
$L = 180, 256$. That gap, together with the $\rho_0 = 3.0$ value
$+3.00$, which exceeds the $d = 2$ ceiling $\chi_{\max} \le N$
that any true exponent must respect, confirms that these are
finite-size crossover rates rather than asymptotic exponents. We
therefore read the density trend qualitatively, as an excess that
grows with $\rho_0$, not as a series of exponents.
...
same patch, giving the $\Delta a \simeq +0.4$ excess. The $U_4$
...
signature here; for the spatial reading we turn instead to
$s_{\rm sep}$.
...
$+0.165$ there) and closing to $+0.01$ by $L = 128$ as they
...
$L = 64$, but within noise of zero at $L = 50, 80$ (CIs
straddling zero, $p \ge 0.56$), so only $L = 64$ is robustly
positive.
...
sizes. Like the susceptibility excess, this $s_{\rm sep}$ surplus
is the noise-shape channel reading out at each mode's own noise;
both express the same dense-phase rectification rather than an
independent cooperation, as the matched-noise control makes
explicit.
...
\alpha_{\max} = 2$ on the same nine-size grid as the
heavy-tailed modes (its row in Table~\ref{tab:slopes}).
It develops no critical $\chi_{\max}(L)$ scaling at our density:
the peak stays in $1.0$--$1.2$ across the nine sizes, slope
...
the Toner--Tu value $\zeta \simeq 0.8$ \citep{TonerTu1995}. The two fixed-noise-shape
...
mode's dense $\Gamma$, erases and reverses the gap:
$\Delta g(r \!\simeq\! 0.6) = -0.07$ ($95\%$ seed-bootstrap CI
$[-0.07, -0.06]$, Wilcoxon $p = 0.002$), the retuned
motility phase now marginally the sharper of the two. The dense
correlation gap therefore measures the lower effective noise the
noise-shape feedback installs inside the dense phase, the
mechanism itself, rather than an alignment the full mode would
keep at equal noise.
...
past $\tau \simeq 100$. The matched-$\Gamma$ control of the
previous section carries over to the dynamics: retuning
motility-only to $\eta = 0.0225$ reverses the dense gap at every
lag, from $\Delta C = -0.08$ at $\tau = 1$ to $-0.42$ at
$\tau = 5\,000$ as the globally quieter retuned mode decorrelates
ever more slowly, so the dynamic dense-coherence advantage, like
the static one, is the visible signature of the rectified dense
noise and not coherence beyond it.
...
0.86$ (Fig.~\ref{fig:decoupled_2d}c) and
\begin{equation}
...
null bounds any persistence difference rather than resolving sub-snapshot dynamics. The
...
dynamics, so it acts on geometry and not on persistence.
...
channel carves out. This is the whole of what the noise-shape
channel does: it lowers the effective angular noise in the dense
phase, and every advantage of the full mode over motility-only
is that single effect read out in a different observable. The
...
$\Gamma$ drives the susceptibility excess $\Delta_9$ from $+0.45$
to $+0.02$, additive within error, and reverses the $g(r)$ and
$C(\tau)$ gaps, the global retune over-quieting the dilute phase. The two adaptations therefore do not
...
dense-phase coherence. Because this priority is a property of the
metric kernel (Sec.~\ref{sec:decoupled}), it applies to
short-range metric perceivers such as marching locusts
\citep{Buhl2006, Bazazi2008}, dense-plume insects and swarming
bacteria, rather than to the topological flockers (starlings,
...
\simeq 0.65$ in both motility-only and full modes (this
single-$\eta$, long-trajectory cumulant is measured at the
$\chi$-peak $\eta$ and differs from the $\eta$-resolved minimum
of Fig.~\ref{fig:pilot}d, which every mode drives to the same
$0.28$--$0.31$ dip and which we already flag as non-diagnostic).
The Toner--Tu programme \citep{TonerTu1995, TonerTu1998,
...
variance. The $\Delta a \simeq +0.4$ gap between the full and
...
bursts out. The motility channel is necessary and heavy tails alone are
not sufficient. What the noise-shape adaptation buys is not a
cooperative excess on top of the motility-driven scaling but a
lower effective noise inside the dense phase, which raises
coherence and susceptibility together through the one route a
matched-noise control isolates. Where \citet{Couzin2002} treats the zonal parameters as
...
than an exponent; and the matched-noise control then shows that
even this residual excess is a noise-level effect ($\Delta_9 \to
+0.02$ on the same nine sizes), not a thermodynamic cooperation. What remains robust is
the qualitative split, which is moreover not a parameter-window
effect, the motility-active growth reproducing away from
$\rho_0 = 2.22$ (Table~\ref{tab:density}).
...
Because the kick enters the heading only modulo $2\pi$
(the update is $\theta \mapsto (\theta_{\rm align} + \xi)
\bmod 2\pi$), the form $\langle\cos\xi\rangle = \exp(-\eta^\alpha)$
is exact rather than small-$\eta$: it already integrates the
wrapped law, and $\Gamma \to 1$ as $\eta^\alpha$ grows, the
wrapped kick saturating to the uniform circle so that no noise can
decorrelate a heading by more than full randomisation. The heavy
...
per cent of every Cauchy step is a near-uniform reorientation that
the Gaussian channel never produces. Driving $\alpha$ from $1$ to
