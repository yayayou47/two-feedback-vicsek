...
otherwise-silent noise-shape adaptation appears at first to add
a super-additive boost on top, but a matched-noise control
dissolves it: once the motility-only mode is retuned so its
heavy-tailed kick decorrelates as fast as the full mode's
rectified one, the dense heading correlation, its dynamic
counterpart $C(\tau)$ and the susceptibility growth all converge,
and the channels become additive. The two feedbacks therefore do
not cooperate beyond what their noise levels dictate; instead the
noise-shape channel acts through a single route, lowering the
effective angular noise inside the motility-built dense phase.
This dense-phase rectification, switching reorientation from
heavy-tailed to Gaussian and the droplets to ballistic transport,
is one mechanism read out statically, dynamically and
thermodynamically at once. A decoupled-sigmoid sweep pins the
motility threshold as the sole control of the sign and
amplitude of the rectification, with the noise-shape threshold
staying passive, and the dominance is a property of the metric
kernel. The coupling thus leaves its mark
not as an emergent thermodynamic synergy but as a density-gated
noise rectification, a signature that should be accessible in
...
Gaussian and an isolated one fast and Cauchy. This single shared
...
$L^{1.04}$ for motility-only, an excess $\Delta_9 = +0.44$,
...
heavy-tailed kick decorrelates as fast as the full mode's
rectified one, drives the excess to $\Delta = +0.05$, additive
...
$a_{\rm full} - a_{\rm motility}$, and the full nine-size fit
returns $\Delta_9 = +0.44$, $95\%$ seed-bootstrap interval
$[+0.31, +0.60]$, when each mode is taken at its own
...
$\alpha = 2$ kick, raises its slope from $a_{\rm motility} =
+1.29$ to $+1.90$ ($[+1.74, +2.26]$), level with
$a_{\rm full} = +1.95$, and collapses the excess to $\Delta =
+0.05$. The faster $\chi_{\max}$ growth of the full mode is thus
nothing more than the lower dense effective noise of its
rectified kick made visible, the same mechanism the dense $g(r)$
and $C(\tau)$ gaps below express; at matched effective noise the
two channels are additive. (The retuned mode is also globally quieter, so this
bounds the excess as a noise effect rather than isolating the
dense phase alone.)
...
at large $L$ under heavy tails. The slope CIs in
...
$a_{\rm full} - a_{\rm motility}$ widens with $\rho_0$.}
\label{tab:density}
\small
...
control collapses all of them together: equalising the dense
decorrelation $\Gamma$ reverses the $g(r)$ and $C(\tau)$ gaps and
drives the susceptibility excess $\Delta$ from $+0.67$ to $+0.05$,
additive within error. The two adaptations therefore do not
...
to $\Delta_9 = +0.44$), confirming a finite-size crossover rather
than an exponent; and the matched-noise control then shows that
even this residual excess is a noise-level effect ($\Delta \to
+0.05$), not a thermodynamic cooperation. What remains robust is
